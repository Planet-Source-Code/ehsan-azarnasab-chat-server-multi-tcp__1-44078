...
'it is important
Dim OneClientIsSending As Boolean
...
Dim User As Integer
    ...
    ElseIf KeyCode = 13 And Shift = 1 Then 'then broad casting
        ...
        For X = 0 To 100
            ...
            If Users(X) <> "" Then
                ...
                'this line is needed because of the winsock bbug!
                DoEvents
            End If
        Next X
        txtSendMessage.Text = ""
...
Dim X As Integer
    ...
    If UserName = "" Then
        ...
        'If there is something in the list, send it to
         RetrieveUser = -1
         If OneClientIsSending = False Then
            MsgBox "Nobody is selected to send", vbExclamation, "Cannot send"
            ' the first one could be selected =)
...
Private Sub wsArray_Close(Index As Integer)
    ' at first prompt other clients he has gone offline
    ' Loop through the users.
    ' There's better ways of doing this
    For X = 0 To 100
        'if there is any user and
        'not to himself!
        If Users(X) <> "" And Users(X) <> Users(Index) Then
          wsArray(X).SendData "o" & Chr(1) & Users(Index)
          DoEvents
        End If
    Next X
...
Dim sON_Clients As String, n As Integer
Dim User As Integer
    ...
    Select Case LCase(CtrlChar)
        Case "f"
        'one of clients have posted msg to another
        'server sees all msg :D
            txtReceived.SelStart = Len(txtReceived.Text)
            n = InStr(Data, Chr(2)) + 1
            txtReceived.SelText = Users(Index) & "->" & Mid(Data, n) & ":" & Left(Data, n - 2) & vbCrLf
            
            OneClientIsSending = True
            User = RetrieveUser(Mid(Data, n))
            OneClientIsSending = False
            'Now if the user is not online we send feedback
            'to the sender!
            If User = -1 Then
                wsArray(Index).SendData "t" & Chr(1) & "Message coudn't be delivered maybe he is offline"
            Else
            'now we send the msg to another
                wsArray(User).SendData "y" & Chr(1) & Users(Index) & Chr(2) & Left(Data, n - 2)
            End If
            DoEvents
        ...
        Case "u"
            ...
            ' There's better ways of doing this
            For X = 0 To 100
               ' If there's a username of the same name listed for them
              If Users(X) = Data Then
                'send feedback to choose another name to login
                wsArray(Index).SendData "t" & Chr(1) & "Please Login With another name:Name is in use"
                'and send command to disconnect
                DoEvents
                wsArray(Index).SendData "a" & Chr(1)
                DoEvents
                wsArray(Index).Close
                DoEvents
                Exit Sub
            ...
            'Data has the name of the new user
            
            
            
            'And User Real Number will be in user
            User = RetrieveUser(Users(Index))
            ...
            ' Now Server will send all ONLine clients to one witch logged
            sON_Clients = "" 'holds the whole ON Clients
            ' Loop through the users.
            ' There's better ways of doing this
            For X = 0 To 100
               ' If there's a username listed for them
              If Users(X) <> "" Then
                sON_Clients = sON_Clients + Users(X) + Chr(2)
              End If
            Next X
            ' Now Our format for our messages is this:
            ' CtrlChar & chr(1) & "Clients1" & chr(2) & "Client2" chr(2),...
            wsArray(User).SendData "r" & Chr(1) & sON_Clients
            DoEvents
            'finally server will send name of this new client to all other
            ' Loop through the users.
            ' There's better ways of doing this
            For X = 0 To 100
               ' If there's a username listed for them
               'in line below User has the number of current user
              If Users(X) <> "" And X <> User Then
                'sending command for new user comming
                wsArray(X).SendData "n" & Chr(1) & Data
                DoEvents
              End If
            Next X
        ...
        Case Else
            txtReceived.SelStart = Len(txtReceived.Text)
            txtReceived.SelText = Users(Index) & ":" & Data & vbCrLf
    End Select
End Sub
